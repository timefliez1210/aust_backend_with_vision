...
struct ScheduleJob {
    ...
    destination_house_number: Option<String>,
    destination_city: Option<String>,
    destination_postal_code: Option<String>,
    estimated_volume_m3: Option<f64>,
    /// Present on every entry type. Whatever the crew is driving to, they must
    /// be able to name the customer and ring them from the card.
    ...
    let mut entries: Vec<ScheduleJob> = job_rows
        ...
        .map(|r| {
            ...
            ScheduleJob {
                ...
                destination_house_number: r.destination_house_number,
...
struct JobDetail {
    ...
    // Zwischenstopp, when the move collects from or drops at a third address
    stop_street: Option<String>,
    stop_house_number: Option<String>,
    stop_city: Option<String>,
    stop_postal_code: Option<String>,
    stop_floor: Option<String>,
    stop_elevator: Option<bool>,
    // Volume
    estimated_volume_m3: Option<f64>,
    items: Vec<ItemInfo>,
...
async fn get_job_detail(
    ...
    Ok(Json(JobDetail {
        ...
        stop_street: row.stop_street,
        stop_house_number: row.stop_house_number,
        stop_city: row.stop_city,
        stop_postal_code: row.stop_postal_code,
        stop_floor: row.stop_floor,
        stop_elevator: row.stop_elevator,
...
struct CalendarItemDetail {
    ...
    category: String,
    location: Option<String>,
    /// A Termin can stand alone, but when it hangs off a customer the crew must
    /// be able to name them and ring them without leaving the page.
    customer_name: Option<String>,
    customer_phone: Option<String>,
...
struct AppointmentDetail {
    ...
    /// The address above came from the linked move, not from this Zusatztermin.
    address_from_inquiry: bool,
    /// The linked move's own endpoints, always sent so the crew has the route.
    inquiry_origin: Option<String>,
    inquiry_destination: Option<String>,
...
async fn get_appointment_detail(
    ...
    Ok(Json(AppointmentDetail {
        ...
        address_from_inquiry: row.address_from_inquiry,
        inquiry_origin: row.inquiry_origin,
        inquiry_destination: row.inquiry_destination,
        customer_name: row.customer_name,
        customer_phone: row.customer_phone,
        notes: assign.notes,
        employee_notes: row.employee_notes,
