...
async fn process_inquiry_background(
    ...
    let s3_keys = services::vision::upload_images_to_s3(&*state.storage, quote_id, estimation_id, &images)
        .await
    ...
    let (total_volume, confidence, result_data, method) =
        match services::vision::try_vision_service(&state, &images, estimation_id, quote_id, estimation_id).await {
            Ok((vol, conf, data)) => {
                tracing::info!(
    ...
    update_quote_volume(&state.db, quote_id, total_volume, "volume_estimated", now_update)
        .await
