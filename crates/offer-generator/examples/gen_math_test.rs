fn main() {
    ...
    let bytes = aust_offer_generator::generate_travel_expense_xlsx(
        &data
    ).expect("should generate");
    std::fs::write("/tmp/reisekosten_math_test.xlsx", bytes).expect("write");
    println!("Written to /tmp/reisekosten_math_test.xlsx");
}
